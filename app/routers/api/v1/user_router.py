...
async def get_user_id(user_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(dependencies.get_current_active_user)):
    return await user_service.get_user_by_id(user_id, db)

@router.post('/new', status_code=201, response_model=GetUser)
async def create_user(user: GetUser, db: AsyncSession = Depends(get_db), current_user: User = Depends(dependencies.get_current_active_user)):
    return await user_service.create_user(user, db)

@router.put('/update', status_code=200)
async def update_user(user_id: int, request: GetUser, db: AsyncSession = Depends(get_db), current_user: User = Depends(dependencies.get_current_active_user)):
